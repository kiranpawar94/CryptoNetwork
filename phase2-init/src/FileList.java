...
import java.util.*;
	...
	// returns a list of files accessible to the groups in the group list 
	public synchronized List<String> getFiles(List<String> groups) {
		List<String> sfl = new ArrayList<String>();
		System.out.println("File group: ");
		for (int i = 0; i < list.size(); i++) {
			if (groups.contains(list.get(i).getGroup())) {
				sfl.add(list.get(i).getPath());
			}
		}
		return sfl;
	}
